# Claude Code 學習資源｜免費教學 + Starter Kit 設定包

> 精選 Claude Code／Codex 中文學習資源，非工程師也能上手。  
> 免費教學文章 + 可直接丟給 AI 執行的設定包 + Mac 一鍵環境安裝包。

🌐 **網站**：[cc.lifehacker.tw](https://cc.lifehacker.tw)  
📬 **教學文 & 電子報**：[raymondhouch.com](https://raymondhouch.com/)  
📦 **最新 Release**：[v1.3 — Mac 一鍵 AI 環境安裝包](https://github.com/Raymondhou0917/claude-code-resources/releases/tag/v1.3)

---

## 這個 Repo 有什麼？

### 📖 學習資源網站

一頁式的 AI Agent 學習入口（[cc.lifehacker.tw](https://cc.lifehacker.tw)），包含：

- **精選免費資源** — 安裝教學、應用案例、技術文件，依場景分類
- **免費導讀** —〈AI Agent 是什麼？〉、〈Mac 一鍵環境安裝包〉圖解（[直接開安裝包教學](https://cc.lifehacker.tw/#lesson-2)）
- **Starter Kit 設定包** — 6 份可直接丟給 AI 執行的設定文件
- **迷你課程導覽** — 由淺入深的系列教學，每篇獨立閱讀
- **應用案例展示** — 非工程師用 Claude Code 實際打造的工具

### 🗺 AI Agent 雙棲知識地圖

如果你正在比較 Claude Code 和 Codex，可以先看這張圖：它把學習路線分成「入門、活用、高手」三層，整理兩者各自優勢，以及 Context、Compact、Skills、SSOT 這些共用底層心法。

![Claude Code × Codex 雙棲進化地圖](images/claude-code-codex-dual-agent-map.png)

### 🧰 環境安裝包（開源・步驟 0）

Mac 一鍵把 git／Homebrew／GitHub 裝好；開場用 checkbox 勾選要裝的 Claude／Codex（桌面版或終端機版，可複選）。不綁課程、已裝好的會自動跳過，可重跑。

- 圖解教學（含 16 張截圖）：[cc.lifehacker.tw/#lesson-2](https://cc.lifehacker.tw/#lesson-2)
- 使用說明：[`env-installer/README.md`](env-installer/README.md)
- 腳本：[`install-mac.sh`](env-installer/install-mac.sh)
- 下載簽名＋公證版 App：[Release v1.3 的 ZIP](https://github.com/Raymondhou0917/claude-code-resources/releases/download/v1.3/raymond-ai-env-installer-mac.zip)

### 🛠 Starter Kit（開源設定包）

每份設定包上半給人看（為什麼需要、裝完會怎樣），下半給 AI 執行（完整指令 + 驗證）。  
丟給 Claude Code 或 Codex 說「照這份文件幫我設定」就能用。支援 **macOS / Linux / Windows**。

AI 執行時會自動：詢問你的個人化偏好 → 偵測既有配置並確認 → 安裝完用白話文條列總結做了什麼。

| # | 設定包 | 解決什麼問題 |
|:--|:--|:--|
| 01 | [終端機優化](starter-kit/01-terminal-setup.md) | 滑鼠支援、減少閃爍、`cc` 快捷啟動（含 Windows + WMUX） |
| 02 | [外部編輯器](starter-kit/02-external-editor.md) | 長指令不再痛苦，用編輯器寫 Prompt |
| 03 | [跨 Agent 安全三件套](starter-kit/03-safe-delete.md) | Claude Code／Codex：垃圾桶 + 危險指令黑名單 + 權限護欄 |
| 04 | [跨 Agent MCP 推薦清單](starter-kit/04-mcp-essentials.md) | Claude Code／Codex 都能讀網頁、操作瀏覽器與串接 Google Workspace |
| 05 | [線上部署](starter-kit/05-deploy-online.md) | 一句話把作品部署上線 |
| 06 | [狀態列設定](starter-kit/06-statusline.md) | 一眼看到模型、額度、Git、時鐘／Session；選配 Fable 5 專屬額度 |

### 🎓 迷你課程

想要更系統性地學？24 小時入門迷你課，從安裝到實戰一次搞定：

→ [**Claude Code 24 小時入門迷你課**](https://lifehacker.tw/courses/24hr-claude-code-tutorial?affcode=git)

想把 AI 練成自己的分身、用中文指揮它處理工作：

→ [**超級 AI 個體｜體驗課**](https://shifu.tw/course/trial/ai-agent-bootcamp)

## 怎麼用？

**方法 1：直接看網站**  
→ [cc.lifehacker.tw](https://cc.lifehacker.tw)

**方法 2：先裝環境（Mac）**  
→ 下載 [環境安裝包 ZIP](https://github.com/Raymondhou0917/claude-code-resources/releases/download/v1.3/raymond-ai-env-installer-mac.zip)，或看圖解 [cc.lifehacker.tw/#lesson-2](https://cc.lifehacker.tw/#lesson-2)

**方法 3：複製 Starter Kit 的 raw 連結，貼給 Claude Code 或 Codex**  
→ 點進任一設定包，複製 raw URL，貼到對話框

**方法 4：看完整 70 分鐘 Claude Code 超詳細入門教學影片**  
→ [https://youtu.be/xo7dE80ktu4](https://youtu.be/xo7dE80ktu4)

## 版本更新

完整紀錄見 [`CHANGELOG.md`](CHANGELOG.md)。

| 版本 | 重點 |
|:--|:--|
| [v1.3](https://github.com/Raymondhou0917/claude-code-resources/releases/tag/v1.3) | Mac 一鍵 AI 環境安裝包（簽名公證 ZIP + 圖解） |
| [v1.2](https://github.com/Raymondhou0917/claude-code-resources/releases/tag/v1.2) | Starter Kit #06 狀態列大升級（Fable 5 額度等） |
| [v1.1](https://github.com/Raymondhou0917/claude-code-resources/releases/tag/v1.1) | Claude Code × Codex 雙棲、Windows 支援 |

## 作者

**[雷蒙三十](https://raymondhouch.com)** — 超級個體、一人公司實踐者。  
經營數位工作術和 AI 應用的線上課程與電子報，用 Claude Code 打造了 20+ 個工具和自動化系統。

- 🌐 [雷蒙公開教學文 & 免費訂閱電子報](https://raymondhouch.com/)
- 📺 [YouTube — 柚智夫妻](https://www.youtube.com/c/yuiraymond)
- 💬 [Threads @raymond0917](https://www.threads.com/@raymond0917)
- 📘 [Facebook](https://www.facebook.com/raymondhou0917)

## 授權

- **Starter Kit 設定包**：[CC BY-NC-SA 4.0](LICENSE) — 歡迎個人使用、學習、分享，禁止商業用途。修改後需以相同授權釋出並標註出處。
- **環境安裝包（env-installer）**：開源、不綁課程；歡迎個人使用與分享。
- **網站內容（文章、課程）**：© 2026 雷蒙三十。歡迎個人學習、筆記引用、社群分享心得，請標註出處。禁止未授權的商業用途或整篇搬運。
