...
> **ver. 3.1** ｜ **Last edited: 2026-07-22**
...
| ☐ Claude 終端機版 | Claude Code，終端機裡的 AI Agent |
| ☐ ChatGPT 桌面版 | ChatGPT 官方桌面 App |
| ☐ Codex 終端機版 | Codex CLI，終端機裡的 AI Agent |

四個選項各自獨立，想裝哪個就勾哪個（例如只要「Claude 終端機版 ＋ ChatGPT 桌面版」也行）。
...
> 📸 每一步都有截圖：**[圖解操作步驟（操作.md）](./操作.md)**

1. 下載本資料夾的 [`雷蒙的 AI 基礎環境安裝包(MAC).zip`](<./雷蒙的 AI 基礎環境安裝包(MAC).zip>)（已簽名＋公證，雙擊即可）。
2. 瀏覽器通常會自動解壓成 App，**雙擊它**。
3. 開場用 ↑↓ 勾選你要的 AI 工具（可複選）、按 Enter，跟著終端機畫面走。中途可能請你輸入 Mac 密碼、或開瀏覽器登入 GitHub——都是正常流程。
...
| `--tools=claude\|codex\|both\|base` | 直接指定要裝什麼，不進選單（適合讓 AI Agent 代跑） |
...
- **一般使用者**：依你選的路線，在終端機打 `claude` 或 `codex`，或打開對應桌面 App。想學怎麼把 AI 練成你的分身、用中文指揮它處理工作？體驗課／課程入口 → [超級 AI 個體體驗課](https://shifu.tw/course/trial/ai-agent-bootcamp)
